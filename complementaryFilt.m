...
cf = complementaryFilter('SampleRate', Fs);
...
% Process sensor data through complementary filter and calculate errors
for i = 1:numSamples
    % Update Complementary filter with current measurements
    estimatedOrientation = cf(accel(i,:), gyro(i,:), mag(i,:));
    
    % Convert the estimated orientation quaternion to a 4-element vector
    estimatedOrientations(i,:) = compact(estimatedOrientation);
    
    % Assuming the gyroscope measures angular velocity directly
